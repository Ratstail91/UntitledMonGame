...
#battle actions system
CREATE TABLE IF NOT EXISTS pendingBattleActions (
	id INTEGER UNSIGNED AUTO_INCREMENT PRIMARY KEY UNIQUE,
	td TIMESTAMP DEFAULT CURRENT_TIMESTAMP(),

	actionType ENUM ('swap', 'move', 'item') DEFAULT 'move',

	battleId INTEGER UNSIGNED NULL DEFAULT NULL,
	userSlotId INTEGER UNSIGNED UNIQUE NULL DEFAULT NULL,

	swapSlot INTEGER UNSIGNED NULL DEFAULT NULL, #incoming swap
	position ENUM ('top', 'bottom') NULL DEFAULT NULL,

	moveActionId INTEGER UNSIGNED NULL DEFAULT NULL,
	targetSlotId INTEGER UNSIGNED NULL DEFAULT NULL,

	itemIdx VARCHAR(100) NULL DEFAULT NULL,

	CONSTRAINT FOREIGN KEY fk_pendingBattleActions_battleId(battleId) REFERENCES battles(id) ON UPDATE CASCADE ON DELETE CASCADE,
	CONSTRAINT FOREIGN KEY fk_pendingBattleActions_userSlotId(userSlotId) REFERENCES battleBoxSlots(id) ON UPDATE CASCADE ON DELETE CASCADE,

	CONSTRAINT FOREIGN KEY fk_pendingBattleActions_swapSlot(swapSlot) REFERENCES battleBoxSlots(id) ON UPDATE CASCADE ON DELETE CASCADE,

	CONSTRAINT FOREIGN KEY fk_pendingBattleActions_moveActionId(moveActionId) REFERENCES creatureMovesOwned(id) ON UPDATE CASCADE ON DELETE CASCADE,
	CONSTRAINT FOREIGN KEY fk_pendingBattleActions_targetSlotId(targetSlotId) REFERENCES battleBoxSlots(id) ON UPDATE CASCADE ON DELETE CASCADE
);