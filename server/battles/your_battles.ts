...
import movesIndex from '../gameplay/moves.json';
...
//TODO: (0) reusable?
export const getYourBattles = (fields) => new Promise(async (resolve, reject) => {
...
//utils
export const getExhaustedItems = async (accountId, battleId) => {
...
export const getEnemyCreatures = async (notAccountId, battleId) => {
...
//utils
export const getStats = async (creature) => {
	if (!creature) {
		return null;
	}

	const slot = (await pool.promise().query('SELECT * FROM battleBoxSlots WHERE creatureId = ?;', [creature.id]))[0][0];
	const moves:any = (await pool.promise().query('SELECT * FROM creatureMovesOwned WHERE creatureId = ? AND equipped = TRUE;', [creature.id]))[0];
